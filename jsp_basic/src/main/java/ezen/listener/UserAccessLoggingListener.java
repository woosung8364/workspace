...
public class UserAccessLoggingListener implements HttpSessionAttributeListener {
    ...
    public void attributeAdded(HttpSessionBindingEvent se)  { 
    	if (se.getName().equalsIgnoreCase("loginName")) {
    		String loginName = (String) se.getValue();
    		System.out.println(loginName + "님이 로그인하셨습니다.");
//    		추후 파일 로그로 변경할 것임...
    		}
    }
    ...
    public void attributeRemoved(HttpSessionBindingEvent se)  { 
    	if (se.getName().equalsIgnoreCase("loginName")) {
    		String loginName = (String) se.getValue();
    		System.out.println(loginName + "님이 로그아웃 하셨습니다.");
//    		추후 파일 로그로 변경할 것임...
    		}
    }
